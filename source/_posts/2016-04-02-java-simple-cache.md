...
在合适的地方使用缓存可以 简化程序逻辑 或者 提高程序的效率. 比如前面的[发送短信](/blog/tags/fa-song-duan-xin/ "发送短信")部分中[限制发送频率](/blog/20160219/sms-java-code-2/ "发送短信(二):限制发送频率")和[限制日发送次数](/blog/20160220/sms-java-code-3/ "发送短信(三):限制日发送次数")就可以使用缓存机制实现.这里我们自己实现一个简易版的缓存类. 该类提供如下功能:
...
至此, 我们的简易版缓存类就算是完成了. 需要完整代码的朋友可以点击[这里](/downloads/code/2016/04/simpleCache.zip "下载代码")下载代码.
